export interface StarterMessage {
  ...
  label: string;
  context: string;
  message: (name: string, product: string, value: string) => string;
}
...
export const starterMessages: StarterMessage[] = [
  ...
  },
  {
    ...
    context: "Cliente acabou de chegar do site",
    message: (name, product, value) =>
      `E aí${name ? ` ${name}` : ""}, bom dia! 😄\nAqui é o Luciano da La Costa.\n\n${product ? `Vi seu interesse em ${product}` : "Vi que você fez uma simulação"}${value ? ` no valor de ${value}` : ""}, que bacana! Me diz uma coisa: tá pesquisando opções ou já tem uma ideia do que quer?`,
  },
  {
    id: "direct-first",
    ...
    context: "Cliente acabou de chegar do site",
    message: (name, product, value) =>
      `Opa${name ? ` ${name}` : ""}, tudo certo? ✌️\nLuciano aqui da La Costa Consórcios!\n\nChegou sua simulação aqui${product ? ` de ${product}` : ""}${value ? ` — ${value}` : ""}. Antes de te mandar as opções, queria entender melhor o que você tá buscando. Me conta!`,
  },
  {
    id: "followup-cold",
